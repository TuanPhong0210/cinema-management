  ...
  return (
    <>
      ...
      <section className="relative mb-8 min-h-[460px] overflow-hidden rounded-[28px] border border-brand-pearl/15 bg-brand-black text-brand-peach shadow-[0_34px_100px_rgba(1,1,1,0.34)]">
        ...
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_42%,rgba(95,67,178,0.92),rgba(95,67,178,0.2)_18%,transparent_34%),linear-gradient(180deg,rgba(1,1,1,0.4),rgba(58,49,83,0.76)_45%,rgba(1,1,1,0.86))]" />
        <div className="absolute left-1/2 top-28 h-36 w-36 -translate-x-1/2 rounded-full bg-brand-studio/80 blur-2xl" />
        <div className="absolute left-1/2 top-28 grid h-32 w-32 -translate-x-1/2 place-items-center rounded-full border border-brand-pearl/20 bg-brand-studio/45 shadow-[0_0_80px_rgba(95,67,178,0.9)] backdrop-blur-md">
          <FontAwesomeIcon icon={faVideo} className="text-4xl text-brand-peach drop-shadow-[0_0_18px_rgba(254,253,253,0.45)]" />
        </div>
        <div className="absolute inset-x-0 bottom-0 h-40 bg-[linear-gradient(rgba(177,174,187,0.16)_1px,transparent_1px),linear-gradient(90deg,rgba(177,174,187,0.12)_1px,transparent_1px)] bg-[size:100%_28px,54px_100%] opacity-70 [transform:perspective(420px)_rotateX(58deg)] [transform-origin:bottom]" />
        <div className="relative flex min-h-[460px] flex-col items-center justify-end px-6 pb-12 text-center md:px-10">
          <p className="text-xs font-bold uppercase tracking-[0.32em] text-brand-pearl">Now Showing</p>
          <h1 className="mt-3 max-w-3xl text-4xl font-extrabold tracking-normal text-brand-peach drop-shadow-[0_0_18px_rgba(254,253,253,0.18)] md:text-6xl">Violet Cinema</h1>
          ...
          <a href="#movie-grid" className="client-btn mt-7 px-6 py-3 text-sm"><FontAwesomeIcon icon={faTicket} /> Browse Movies</a>
        </div>
      </section>
      ...
      <div className="client-surface mb-8 rounded-[18px] p-4">
        <div className="grid gap-3 md:grid-cols-[1fr_auto]">
          ...
          <div className="flex flex-wrap gap-2">
            {genres.map((item) => <button key={item} onClick={() => setGenre(item)} className={item === genre ? 'inline-flex items-center justify-center rounded-full bg-brand-studio px-4 py-2.5 text-sm font-semibold text-brand-peach shadow-[0_0_22px_rgba(95,67,178,0.34)] transition hover:bg-brand-studio/80' : 'inline-flex items-center justify-center rounded-full border border-brand-pearl/20 bg-brand-black/20 px-4 py-2.5 text-sm font-semibold text-brand-pearl transition hover:border-brand-studio hover:text-brand-peach'}>{item}</button>)}
          </div>
        </div>
      </div>
