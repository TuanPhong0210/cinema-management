...
const links = [
  ...
  ['Movies', '/movies', faFilm],
  ['Rooms', '/rooms', faVideo],
  ['Showtimes', '/showtimes', faClock],
  ['Tickets', '/tickets', faTicket],
  ['Employees', '/employees', faUsers],
  ['Attendance', '/attendance', faUserTie]
];
      ...
      <aside className="bg-cinema-950 text-white lg:fixed lg:inset-y-0 lg:w-72">
        <div className="flex h-16 items-center justify-between px-5 lg:h-20">
          <div>
            ...
            <a href="http://localhost:5173/" className="text-xl font-extrabold hover:text-violet-200 transition">Violet Cinema</a>
          </div>
        ...
        <nav className="flex gap-2 overflow-x-auto px-3 pb-3 lg:block lg:space-y-1 lg:overflow-visible">
          {links.map(([label, to, icon]) => (
            <NavLink key={to} to={to} end={to === '/'} className={({ isActive }) => `flex shrink-0 items-center gap-3 rounded-md px-4 py-3 text-sm font-semibold ${isActive ? 'bg-white text-cinema-900' : 'text-violet-100 hover:bg-white/10'}`}>
